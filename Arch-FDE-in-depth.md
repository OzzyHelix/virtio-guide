...
echo "cryptroot UUID=$(blkid -s UUID -o value /dev/nvme0n1p1) none  luks,discard,no-read-workqueue,no-write-workqueue" >> /mnt/etc/crypttab
...
# or initrd /amd-ucode.img if you have an AMD cpu
...
sudo pacman -S nvidia-dkms hip-runtime-nvidia lib32-nvidia-utils lib32-opencl-nvidia libva-nvidia-driver nvidia-settings nvidia-utils opencl-nvidia
```
